...
class AdaptiveSampler2D:
    ...
    def init_pdf(self):
        ...
        pdf = np.ones((num_of_cells, num_of_cells)) / num_of_cells ** 2  # uniform random
        return pdf
    ...
    def run(self, num_iterations, print_every=0, uniform=False):
    ...
    def get_decision_regions(self):
        pred_X_free, pred_X_obs = set(), set()
        for theta_1 in self.angle_range:
            for theta_2 in self.angle_range:
                x = (theta_1, theta_2)
                pred = self.classifier(x)
        ...
        return (pred_X_obs, pred_X_free)

    def visualize_pdf(self):
        plt.imshow(self.pdf, cmap='viridis', origin='lower', extent=[0, 2 * np.pi, 0, 2 * np.pi])
